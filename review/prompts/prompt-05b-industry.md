...
`domains/healthcare.md`. Part 8 (from 05a) is reused unchanged across domains.

The output filename is `[[FRAMEWORK_LOWER]]_review_05_maturity_financial.md` —
the `_financial` suffix is a **historical artefact** retained for backwards
compatibility with downstream agents (prompt-06 reads Part 8 from this file;
prompt-08 reads both Part 8 and Part 9). Downstream agents need **no modification**.
...
`[[DOMAIN_FILE]]` is `domains/healthcare.md`, only healthcare regulations appear.
If `[[DOMAIN_FILE]]` is `domains/insurance.md`, only insurance regulations appear.
...
1. **Filename-invariance disclaimer** (mandatory first content, blockquote line,
   verbatim):

   ```
   > Note: This file is named `_review_05_maturity_financial.md` for historical reasons, regardless of the domain under review. Part 9 content adapts to [[DOMAIN_FILE]].
   ```

2. **Header metadata block** (re-emit using the canonical combined-file format —
   see Required Structure below). Include `[[INDUSTRY]]` and `[[ORGANIZATION]]` here
   even though 05a's header did not.

3. **Part 8 — Maturity Phase Placement** — copied verbatim from 05a's output
...
4. **Part 9 — [[INDUSTRY]] Assessment ([[ORGANIZATION]])** — produced here per the
   methodology above.
...
- **No ASDLC / APLC references — including forward-propagation from `[[DOMAIN_FILE]]`.**
  ...
  `agentic-sdlc-handbook/` anywhere in the output. **When `[[DOMAIN_FILE]]` itself
  references APLC or ASDLC mechanisms** (e.g., `domains/insurance.md` references
  "the APLC's behavioural specification"), **paraphrase to manifesto-equivalent
  terms** (e.g., "APLC behavioural specification" → "the framework's specification
  artefact"; "ASDLC phase gate" → "the framework's phase gate"). Do NOT propagate
  APLC/ASDLC vocabulary, paths, or filenames into the output even when they appear
  in the cited domain file.
...
- [ ] Output file is written to
      `[[FRAMEWORK_LOWER]]/[[FRAMEWORK_LOWER]]_review_05_maturity_financial.md`
      (the canonical combined file name — historical artefact preserved for
      downstream agents).
- [ ] Output file begins with the filename-invariance disclaimer blockquote
      (`> Note: This file is named ...`) verbatim, before the H1.
