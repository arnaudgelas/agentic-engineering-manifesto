...
- The `adoption/vmodel.md` subsection (Part 6) assesses whether [[FRAMEWORK]] *artefacts* satisfy V-model levels. Cross-reference Part 3 (the loop phase analysis) only via canonical part number. Do not re-derive loop-phase content.
...
**Severity labels and alignment grades operate on different things.** Part 6 does NOT use 0–100 scores. Use alignment grades (see below) at the subsection level, and severity labels (Critical/High/Medium/Low) for individual gaps. Because there is no numeric score to threshold, the score-band table in `prompt.md` does not apply here: apply the **regulatory-impact rubric** from `prompt.md` § "Severity for findings that carry no score", which the orchestrator supplies in the Universal Prepend Block. Do not re-quote or re-define either table here. State which tier applies and why in one clause per gap.
...
**Downstream use:** Agent 09 merges this file's `## Part 6` into the review **unchanged** — nothing rewrites it. Agent 04c reads it (with `[[FRAMEWORK_LOWER]]_review_04b_companion.md`) to write the Cross-Document Synthesis and merge the gap inventories. Write Part 6 to publication standard.
```
...
4. Every [[ORGANIZATION]]-specific implication must map to a specific regulation Article (with Article or section number) or a named risk-type entry from `[[DOMAIN_FILE]]`.
5. The synthesis-level subsection reference form is `Part 6 — <file-slug>`.
6. Do not propagate `[[DOMAIN_FILE]]` content forward into unrelated synthesis (no domain-file bleed beyond cited regulations and risk-types).
7. Make falsifiable statements grounded in cited artefacts. If evidence is uncertain, state `unverified — source artefact does not address X`.
8. Do not produce a composite [[FRAMEWORK]] score. Part 1 is assembled by agent 09 from the deep files.
9. Do not produce Part 7 content (companion alignment) or the Cross-Document Synthesis. Those are owned by agents 04b and 04c respectively.
10. Subsection order within Part 6 is canonical (per §3.4) and must not be re-ordered.
11. Close the output file with an italic `*Sources read: ...*` footer listing every source file actually read.
...
*Sources to read: all files in `[[FRAMEWORK_PATH]]`; `adoption/path.md`; `adoption/playbook.md`; `adoption/enterprise.md`; `adoption/metrics.md`; `adoption/roles.md`; `adoption/pilot.md`; `adoption/vmodel.md`; `[[DOMAIN_FILE]]`; and `[[PRIOR_REVIEWS]]` if not `none`.*
