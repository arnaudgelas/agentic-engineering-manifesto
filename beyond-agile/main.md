...
Current industry forecasts and long-horizon maintenance benchmarks suggest that agentic delivery fails when teams force probabilistic systems through legacy SDLC workflows <sup>[8](beyond-agile-sources.md#ref-8)</sup><sup>[22](beyond-agile-sources.md#ref-22)</sup>. The signal is strongest for maintenance-heavy, multi-iteration work, not every software activity. Methodology mismatch is one failure mode among several; cost, governance, tool quality, and organizational incentives also matter.
...
The industry is repeating the pattern it followed with every previous paradigm shift: rushing to adopt the new technology while clinging to the old methodology. Fixed-length sprints — whether two weeks or six — do not accommodate machine-speed execution. Story points do not measure probabilistic output. Human code review does not scale to agent-generated volume. Standups do not synchronize digital swarms. Early empirical evidence confirms the scale of the problem: the SWE-CI benchmark, testing 18 models across 100 tasks spanning an average of 233 days of real development history, found that most evaluated models achieve zero-regression rates below 0.25 — many of the regressions they do introduce are structural, passing current tests but degrading the codebase's capacity for future change <sup>[22](beyond-agile-sources.md#ref-22)</sup> (arXiv:2603.03823v1).
